...
**OpenNext incremental cache (`open-next.config.ts`) is what makes the prerendered shell cheap to serve.** A bare `defineCloudflareConfig()` defaults `incrementalCache`/`queue` to `"dummy"` — nothing persists, so every request re-renders the route in the Worker and re-runs the Sanity fetches, PPR shell or not (this was the root cause of issue #6's TTFB). The config wires `incrementalCache` to R2 (not KV — KV's ~60s global write propagation would blow the ~30s freshness window above) wrapped in `withRegionalCache({ mode: "long-lived" })` for warm-region reuse, plus `queue: "direct"` so revalidation actually fires — a cache with the dummy queue just freezes content at deploy time. `enableCacheInterception` must stay `false`: the package docs call it out as unsupported alongside PPR/`cacheComponents`. The R2 bucket binding is `NEXT_INC_CACHE_R2_BUCKET` (`wrangler.jsonc`, fixed by the override, not configurable) and gets populated automatically by `npm run deploy:cf` (`opennextjs-cloudflare deploy` seeds the cache before `wrangler deploy`) — no manual seeding step beyond the one-time `wrangler r2 bucket create`.
